...
# Validating rule cloudfront_https: Deny CloudFront Distributions that are not enforcing HTTPS or Redirect to HTTPS Viewer Protocol Policy
...
test_cloudfront_distribution_https {
	...
	count(deny) == 1 with input as resources["aws_cloudfront_distribution.invalid"]
	count(deny) == 0 with input as resources["aws_cloudfront_distribution.valid"]
}

# Mock input is generated plan for cloudfront_https.tf
mock_plan_input = {
	"format_version": "0.1",
